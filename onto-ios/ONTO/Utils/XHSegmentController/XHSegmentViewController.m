...
- (XHSegmentControl *)segmentControl
{
    if (!_segmentControl)
    {
        ...
        CGFloat y = 0;
        if (USERMODEL.isShareWallet ==YES) {
            y =170*SCALE_W;
        }
...
- (void)setSegmentControlTop{
    
    CGRect rectStatus = [[UIApplication sharedApplication] statusBarFrame];

    _segmentControl.frame = CGRectMake(0, -20, [UIScreen mainScreen].bounds.size.width, DefaultSegmentHeight);
}
...
- (UIScrollView *)scrollView
{
    ...
    if (!_scrollView)
    {
        _scrollView = [[UIScrollView alloc] initWithFrame:CGRectMake(0, CGRectGetMaxY(_segmentControl.frame), [UIScreen mainScreen].bounds.size.width, [UIScreen mainScreen].bounds.size.height - CGRectGetMaxY(_segmentControl.frame)-LL_TabbarSafeBottomMargin-LL_StatusBarAndNavigationBarHeight)];
