...
#import "ONTODAppWalletTipsAlertView.h"
...
@interface ONTODAppWalletTipsAlertView ()
{
    NSString           *_titleStr;
}
...
@end
@implementation ONTODAppWalletTipsAlertView
#pragma mark - Init
+(ONTODAppWalletTipsAlertView *)CreatWalletTipsAlertView
{
    return [[ONTODAppWalletTipsAlertView alloc] init];
}
...
//        make.height.mas_equalTo(59);
    ...
    [self.inputButton mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.mas_equalTo(20);
        make.top.equalTo(self.createButton.mas_bottom).offset(15);
        make.height.mas_equalTo(42);
        make.right.equalTo(self.bgView).offset(-20);
...
-(void)p_initData
{
    if (_isDapp) {
        ...
        _titleStr = Localized(@"experienceDApp");
    }else{
        _titleStr = Localized(@"experienceWake");
    }
...
-(void)p_inputSlot:(UIButton*)button
{
    [self p_removeFromSuperView];
    
    if (self.delegate&&[self.delegate respondsToSelector:@selector(walletTipsAlertInputBtnSlot:)])
    {
        [self.delegate walletTipsAlertInputBtnSlot:self];
    }
...
-(void)p_createSlot:(UIButton*)button
{
    ...
    if (self.delegate&&[self.delegate respondsToSelector:@selector(walletTipsAlertCreateBtnSlot:)])
    {
        [self.delegate walletTipsAlertCreateBtnSlot:self];
    }
}
...
-(UIButton*)createButton
{
    if (!_createButton)
    {
        ...
        [_createButton setTitle:Localized(@"DappCreateWalletTitle") forState:UIControlStateNormal];
...
-(UIButton*)inputButton
{
    if (!_inputButton)
    {
        ...
        [_inputButton setTitle:Localized(@"DappInputWalletTitle") forState:UIControlStateNormal];
        _inputButton.titleLabel.font = [UIFont boldSystemFontOfSize:14];
        [_inputButton setTitleColor:[UIColor colorWithHexString:@"#000000"] forState:UIControlStateNormal];
        _inputButton.backgroundColor = [UIColor colorWithHexString:@"#ffffff"];
        _inputButton.clipsToBounds = YES;
        _inputButton.layer.cornerRadius = 5;
        [_inputButton addTarget:self action:@selector(p_inputSlot:) forControlEvents:UIControlEventTouchUpInside];
    }
    return _inputButton;
}
