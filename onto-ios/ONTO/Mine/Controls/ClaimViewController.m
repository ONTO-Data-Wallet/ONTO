...
#import "Common.h"
#import "ONTO-Swift.h"
@interface ClaimViewController ()<UITableViewDelegate, UITableViewDataSource, WKNavigationDelegate>
...
- (void)handleData:(NSDictionary *)responseObject {
  ...
  if ([Common isBlankString:[responseObject valueForKey:@"TxnHash"]]) {
    ...
    NSArray *arrKey = @[Localized(@"authStatus"), Localized(@"Created"), Localized(@"ExpiresAt")];
    NSArray *arrValue = @[statusStr,
    ...
    if (statusStr != nil && TxnHash != nil && created != nil && exp != nil) {
      NSArray *arrValue = @[statusStr, created, exp, TxnHash];
      [_contentTitleArr addObject:arrKey];
      [_contentValueArr addObject:arrValue];
    }
  }
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
  ...
  WebIdentityViewController *vc = [[WebIdentityViewController alloc] init];
  vc.transaction = self.contentValueArr[1][3];
  [self.navigationController pushViewController:vc animated:YES];
}
