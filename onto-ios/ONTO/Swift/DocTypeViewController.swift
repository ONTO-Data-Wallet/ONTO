...
class DocTypeViewController: BaseViewController {
    ...
    func configUI()  {
        ...
        for model in self.modelArr {
            ...
            if model1.claimContext == "claim:idm_idcard_authentication"{
                statusArr.replaceObject(at: 0, with: model1.issuedFlag)
            }else if model1.claimContext == "claim:idm_passport_authentication"{
                statusArr.replaceObject(at: 1, with: model1.issuedFlag)
            }else if model1.claimContext == "claim:idm_dl_authentication"{
        ...
        let path2 = Bundle.main.path(forResource: "unHave", ofType: "json")
...
//        country =    Common.getJsLocaleWithcode(country)
        ...
        for i in 0..<3{
            ...
            card1.settype(type: i,claim: false)
    ...
    @objc func showZoomImageView( tap : UITapGestureRecognizer) {
        ...
        if tapView.cardStatus1 == 9 {
            ...
            if tap.view?.tag == 0 {
                ...
                for model in self.modelArr {
                    let model1:IdentityModel = model as! IdentityModel
                    if model1.claimContext == "claim:idm_passport_authentication"{
                        vc.shuftiModel = model1
                    }
                }
                ...
                for model in self.modelArr {
                    let model1:IdentityModel = model as! IdentityModel
                    if model1.claimContext == "claim:idm_dl_authentication"{
                        vc.shuftiModel = model1
                    }
                }
            }
            ...
            for model in self.modelArr {
                ...
                if model1.claimContext == "claim:idm_passport_authentication"{
                    ...
                    vc.shuftiModel = model1
                }else if model1.claimContext == "claim:idm_idcard_authentication"{
                    vc.shuftiModel = model1
                }
            }
            self.navigationController?.pushViewController(vc, animated: true)
