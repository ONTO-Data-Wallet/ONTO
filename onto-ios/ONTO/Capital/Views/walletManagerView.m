...
@property (nonatomic,   copy) void(^defaultWalletBlock)(NSIndexPath *);
@end

@implementation walletManagerView
-(instancetype)initWithOptionsArr:(NSArray *)optionsArr shareArray:(NSArray *)shareArray cancelTitle:(NSString *)cancelTitle defaultWalletBlock:(void (^)(NSIndexPath *))defaultWalletBlock selectedBlock:(void (^)(NSIndexPath *))selectedBlock{
    if (self = [super init]) {
        _optionsArr = optionsArr;
        _shareArray = shareArray;
        _cancelTitle = cancelTitle;
        _selectedBlock = selectedBlock;
        _defaultWalletBlock =defaultWalletBlock;
        NSString *str =[NSString stringWithFormat:@"%@",Localized(@"CreateWallet")];
        self.actionArr = @[@{@"image":@"newcreatewallet",@"title":str},@{@"image":@"newimportwallet",@"title":Localized(@"ImportWallet")},@{@"image":@"newmanagewallet",@"title":Localized(@"ManageWallet")},@{@"image":@"newcreatejoinwallet",@"title":Localized(@"createShareWallet")},@{@"image":@"newjoinsharedwallet",@"title":Localized(@"leadShareWallet")}];
        [self craetUI];
    }
    return self;
}
...
- (void)craetUI {
    ...
    [self.topView addSubview:self.collectionView2];
...
-(UICollectionView*)collectionView2{
    if (!_collectionView2) {
        ...
        _collectionView2 = [[UICollectionView alloc ]initWithFrame:CGRectMake(0, 132*SCALE_W, SYSWidth-16*SCALE_W, 132*SCALE_W) collectionViewLayout:flowLayout] ;
        _collectionView2.delegate = self;
        _collectionView2.dataSource = self;
        _collectionView2.clipsToBounds =YES;
        _collectionView2.backgroundColor = [UIColor whiteColor];
        _collectionView2.showsHorizontalScrollIndicator =NO;
        [_collectionView2 registerClass:[shareManagerCell class] forCellWithReuseIdentifier:@"shareManager"];
    }
    return _collectionView2;
}
-(UIView*)bgView{
    if (!_bgView) {
        _bgView =[[UIView alloc]initWithFrame:CGRectMake(8*SCALE_W, SYSHeight-331*SCALE_W , Screen_Width - (SCALE_W * 16), 331*SCALE_W)];
...
-(UIView*)topView{
    if (!_topView) {
        _topView =[[UIView alloc]initWithFrame:CGRectMake(0,0, Screen_Width - (SCALE_W * 16), 264*SCALE_W)];
...
-(UIButton*)cancelBtn{
    if (!_cancelBtn) {
        _cancelBtn =[[UIButton alloc]initWithFrame:CGRectMake(0, 272*SCALE_W, SYSWidth -16*SCALE_W, 50*SCALE_W)];
...
-(void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath{
    if ([collectionView isEqual:_collectionView]) {
        self.defaultWalletBlock(indexPath);
    }else{
...
- (void)show {
    ...
    _bgView.frame = CGRectMake(8*SCALE_W, y , Screen_Width - (SCALE_W * 16), 331*SCALE_W);
    
    [UIView animateWithDuration:.2 animations:^{
        _bgView.frame = CGRectMake(8*SCALE_W, SYSHeight-331*SCALE_W , Screen_Width - (SCALE_W * 16), 331*SCALE_W);
        _maskView.alpha = .5;
