   ...
   WalletListCell *cell = [[[NSBundle mainBundle] loadNibNamed:@"WalletListCell" owner:self options:nil] lastObject];
    ...
    if (indexPath.section == 0) {
        ...
        if (indexPath.row==[[[NSUserDefaults standardUserDefaults]valueForKey:SELECTWALLET] integerValue]) {
            cell.nameLabel.textColor =  [UIColor colorWithHexString:@"#0AA5C9"];
            cell.rightImage.image =     [UIImage imageNamed:@"blue"];
            cell.backgroundColor = [UIColor colorWithHexString:@"#F6F8F9"];
        }
        
    }  else if (indexPath.section==1&&indexPath.row==0) {
        
        cell.rightImage.image = [UIImage imageNamed:@"icon_create_s_blue copy 3"];
        cell.nameLabel.text = Localized(@"CreateWallet");
        
    }else if (indexPath.section==1&&indexPath.row==1) {
        cell.rightImage.image = [UIImage imageNamed:@"dark 2"];
        cell.nameLabel.text = Localized(@"ImportWallet");
    }else if (indexPath.section==1&&indexPath.row==2) {
        cell.rightImage.image = [UIImage imageNamed:@"manage"];
        cell.nameLabel.text = Localized(@"ManageWallet");
...
//        cell.nameLabel. = Screen_Width/2;
    ...
    if (indexPath.section==1) {
        ...
        cell.nameLabel.textColor = [UIColor colorWithHexString:@"#6A797C"];
        if (indexPath.row==0) {
